...
from .models import DoctorReg, Specialization
...
def Search_Appointments(request):
    ...
    if request.method == "GET":
        ...
        if query:
            ...
            patient = Appointment.objects.filter(fullname__icontains=query) | Appointment.objects.filter(appointment_number__icontains=query) & Appointment.objects.filter(doctor_id=doctor_reg)
...
def Between_Date_Report(request):
    ...
    return render(request, 'doc_profile/between-dates-report.html', {'patient': patient, 'start_date': start_date, 'end_date':end_date})
